...
Singleton {
    ...
    /**
     * Normalizes a time value that might be in seconds, milliseconds, or microseconds.
     * MPRIS spec says microseconds, but some players or wrappers might use seconds or millis.
     * @param { number } time
     * @returns { number }
     */
    function normalizeTime(time) {
        ...
        // Thresholds:
        // Nanoseconds: > 1,000,000,000,000 (10^12) -> 1,000s = 16.6m
        // Microseconds: > 2,000,000 (2*10^6) -> 2s
        
        if (time > 1000000000) {
            // Likely Nanoseconds or very large Microseconds.
            // If it's > 10^12, it's definitely Nanos.
            // Some players (Harmonoid?) might use Nanos.
            if (time > 100000000000) return time / 1000000000;
            return time / 1000000;
        }
        
        // If it's between 0 and 1,000,000,000, we treat it as seconds.
        return time;
    }

    /**
     * Normalizes a volume value. MPRIS spec says 0.0-1.0.
     * Some players (Harmonoid) use 0-10000.
     * @param { number } volume
     * @returns { number }
     */
    function normalizeVolume(volume) {
        ...
        if (volume > 1.5) {
            // If > 1.5, it's likely not the standard 0.0-1.0 range.
            // Check if it's 0-100 or 0-10000.
            if (volume > 150) return volume / 10000;
            return volume / 100;
        }
        return volume;
    }
