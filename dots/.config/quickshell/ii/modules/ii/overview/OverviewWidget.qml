...
Item {
    ...
    property int minWorkspaceWidth: (monitorData?.transform % 2 === 1) ? 
        ((monitor.height - monitorData?.reserved[0] - monitorData?.reserved[2]) * root.scale) :
        ((monitor.width - monitorData?.reserved[0] - monitorData?.reserved[2]) * root.scale)
    ...
    readonly property list<int> workspaceMap: Config.options.overview.workspaceMap ?? [0]
    property int monitorIndex: -1 // to be set by parent
    property int workspaceOffset: (useWorkspaceMap && monitorIndex >= 0 && monitorIndex < workspaceMap.length) ? workspaceMap[monitorIndex] : 0
    ...
    property real workspaceImplicitHeight: (monitorData?.transform % 2 === 1) ? 
        ((monitor.width - (monitorData?.reserved?.[1] ?? 0) - (monitorData?.reserved?.[3] ?? 0)) * root.scale) :
        ((monitor.height - (monitorData?.reserved?.[1] ?? 0) - (monitorData?.reserved?.[3] ?? 0)) * root.scale)
