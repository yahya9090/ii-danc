...
Item {
    ...
    property real dateSquareSize: 64 * sizeMultiplier
    ...
    FadeLoader {
        ...
        anchors {
            ...
            right: parent.right
            rightMargin: (40 - rectLoader.opacity * 30) * root.sizeMultiplier
        }

        sourceComponent: RectangleDate {
            color: ColorUtils.mix(root.color, Appearance.colors.colSecondaryContainerHover, 0.5)
            radius: Appearance.rounding.small
            implicitWidth: 45 * rectLoader.opacity * root.sizeMultiplier
            implicitHeight: 30 * rectLoader.opacity * root.sizeMultiplier
            sizeMultiplier: root.sizeMultiplier
        }
    }
