...
Singleton {
    ...
    property list<var> availableTools: {
        const model = models[currentModelId];
        if (!model || !root.tools[model.api_format]) return [];
        return Object.keys(root.tools[model.api_format]);
    }
    ...
    function addExtraModels() {
        (Config?.options.ai?.extraModels ?? []).forEach(model => {
            const safeName = safeModelName(model["model"]);
            addModel(safeName, model)
        });
    }
    ...
    Component.onCompleted: {
        ...
        addExtraModels() // Config onReadyChanged above might not fire if config is loaded before this service
    }
