...
Singleton {
    ...
    function incrementVolume() {
        ...
        const configStep = (Config.options.audio.volumeStep || 2) / 100;
        const step = currentVolume < 0.1 ? Math.min(0.01, configStep) : configStep;
        Audio.sink.audio.volume = Math.min(root.hardMaxValue, Audio.sink.audio.volume + step);
    }
