...
Singleton {
    ...
    property string filterType: "all" // "all", "static", "animated"
    readonly property list<string> staticExtensions: ["jpg", "jpeg", "png", "webp", "avif", "bmp", "svg"]
    readonly property list<string> animatedExtensions: ["mp4", "mkv", "webm", "avi", "mov", "m4v", "ogv", "gif"]
    readonly property list<string> extensions: [
        "jpg", "jpeg", "png", "webp", "avif", "bmp", "svg", "mp4", "mkv", "webm", "avi", "mov", "m4v", "ogv", "gif"
    ]
    ...
    function apply(path, darkMode = Appearance.m3colors.darkmode) {
        ...
        const trimmedPath = FileUtils.trimFileProtocol(path);
        applyProc.exec([
            Directories.wallpaperSwitchScriptPath,
            "--image", trimmedPath,
            "--mode", (darkMode ? "dark" : "light")
    ...
    FolderListModelWithHistory {
        ...
        caseSensitive: false
        nameFilters: {
            let baseExts = root.extensions;
            if (root.filterType === "static") baseExts = root.staticExtensions;
            else if (root.filterType === "animated") baseExts = root.animatedExtensions;

            return baseExts.map(ext => `*${searchQuery.split(" ").filter(s => s.length > 0).map(s => `*${s}*`)}*.${ext}`)
        }
        showDirs: true
