            ...
            if (!usingSearch && message.functionCall != undefined && message.functionName.length > 0) {
                let part = {
                    functionCall: {
                        ...
                        "args": message.functionCall.args // Assuming args are stored
                    }
                }
                if (message.thoughtSignature && message.thoughtSignature.length > 0) {
                    part.functionCall.thought_signature = message.thoughtSignature;
                }
                return {
